...
// CaliforniaStatePortal SDK feature test
//
...
class FtCtrl extends CaliforniaStatePortalControl
{
...
class FtClient
{
    ...
    private CaliforniaStatePortalUtility $utility;

    public function __construct(CaliforniaStatePortalUtility $utility, array $options)
    {
    ...
    public function get_utility(): CaliforniaStatePortalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CaliforniaStatePortalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CaliforniaStatePortalContext $rootctx;

    // True when this SDK was generated with the named feature.
    public static function hasFeature(string $name): bool
    {
        if (self::$feature_config === null) {
            $config = CaliforniaStatePortalConfig::make_config();
            $f = $config['feature'] ?? [];
    ...
    public function __construct(
        ...
        $utility = new CaliforniaStatePortalUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (CaliforniaStatePortalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CaliforniaStatePortalConfig::make_feature($name);
    ...
    private static function buildUrl(CaliforniaStatePortalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CaliforniaStatePortalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CaliforniaStatePortalResponse($fetched);
        }
    ...
    private function fail(CaliforniaStatePortalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CaliforniaStatePortalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
