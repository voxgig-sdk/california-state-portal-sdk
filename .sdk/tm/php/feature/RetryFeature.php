...
// CaliforniaStatePortal SDK retry feature
...
class CaliforniaStatePortalRetryFeature extends CaliforniaStatePortalBaseFeature
{
    ...
    public function init(CaliforniaStatePortalContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CaliforniaStatePortalContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CaliforniaStatePortalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CaliforniaStatePortalContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
