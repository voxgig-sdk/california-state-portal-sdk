...
// CaliforniaStatePortal SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CaliforniaStatePortalTestFeature extends CaliforniaStatePortalBaseFeature
{
    ...
    public function init(CaliforniaStatePortalContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CaliforniaStatePortalContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CaliforniaStatePortalParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CaliforniaStatePortalContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CaliforniaStatePortalContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CaliforniaStatePortalParam::call($ctx, $k);
