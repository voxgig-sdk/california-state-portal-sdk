...
// CaliforniaStatePortal SDK pipeline test
//
...
{
    private static function utility(): CaliforniaStatePortalUtility
    {
        return new CaliforniaStatePortalUtility();
    }
    ...
    private static function ctx(array $over = []): CaliforniaStatePortalContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CaliforniaStatePortalContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CaliforniaStatePortalError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CaliforniaStatePortalMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CaliforniaStatePortalMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CaliforniaStatePortalMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CaliforniaStatePortalSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CaliforniaStatePortalMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = CaliforniaStatePortalMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec([]),
            'response' => null,
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        [, $err] = CaliforniaStatePortalMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec(['step' => 's']),
            'response' => new CaliforniaStatePortalResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        [, $err] = CaliforniaStatePortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec(['step' => 's']),
            'response' => new CaliforniaStatePortalResponse(self::resp(200, ['v' => 1])),
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        [, $err] = CaliforniaStatePortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec(['step' => 's']),
            'response' => new CaliforniaStatePortalResponse(self::resp(200, ['v' => 2])),
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CaliforniaStatePortalMakeResponse::call($ctx);
    ...
    {
        $preset = new CaliforniaStatePortalResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec([]),
            'response' => new CaliforniaStatePortalResponse([]),
            'result' => new CaliforniaStatePortalResult([]),
        ...
        [$got, $err] = CaliforniaStatePortalMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CaliforniaStatePortalResult([])]);
        [, $err] = CaliforniaStatePortalMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CaliforniaStatePortalSpec([]), 'result' => null]);
        [, $err] = CaliforniaStatePortalMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CaliforniaStatePortalSpec(['step' => 's']),
            'result' => new CaliforniaStatePortalResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CaliforniaStatePortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CaliforniaStatePortalSpec(['step' => 's']),
            'result' => new CaliforniaStatePortalResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CaliforniaStatePortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CaliforniaStatePortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CaliforniaStatePortalResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CaliforniaStatePortalSpec([]),
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CaliforniaStatePortalMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CaliforniaStatePortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CaliforniaStatePortalError('boom', 'boom');
        $utility->fetcher = function (CaliforniaStatePortalContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CaliforniaStatePortalSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CaliforniaStatePortalMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CaliforniaStatePortalContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CaliforniaStatePortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CaliforniaStatePortalResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CaliforniaStatePortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CaliforniaStatePortalContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CaliforniaStatePortalSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CaliforniaStatePortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CaliforniaStatePortalResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CaliforniaStatePortalSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CaliforniaStatePortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CaliforniaStatePortalMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CaliforniaStatePortalMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CaliforniaStatePortalResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CaliforniaStatePortalDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CaliforniaStatePortalResult(['ok' => false])]);
        $this->expectException(CaliforniaStatePortalError::class);
        CaliforniaStatePortalDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CaliforniaStatePortalResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CaliforniaStatePortalMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CaliforniaStatePortalMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CaliforniaStatePortalBaseFeature();
        $b = new CaliforniaStatePortalBaseFeature();
        CaliforniaStatePortalFeatureAdd::call($ctx, $a);
        CaliforniaStatePortalFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CaliforniaStatePortalBaseFeature
    {
        $f = new CaliforniaStatePortalBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CaliforniaStatePortalFeatureAdd::call($ctx, self::named_feature('a'));
        CaliforniaStatePortalFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CaliforniaStatePortalFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CaliforniaStatePortalFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CaliforniaStatePortalFeatureAdd::call($ctx, $replace);
        ...
        CaliforniaStatePortalFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CaliforniaStatePortalContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CaliforniaStatePortalSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CaliforniaStatePortalPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CaliforniaStatePortalPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CaliforniaStatePortalResponse(['headers' => null]),
            'result' => new CaliforniaStatePortalResult([]),
        ]);
        CaliforniaStatePortalResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CaliforniaStatePortalResultBody::call($ctx);
