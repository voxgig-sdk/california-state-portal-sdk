...
class ServiceDirectTest extends TestCase
{
    public function test_direct_list_service(): void
    {
        $setup = service_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-service", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function service_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new CaliforniaStatePortalSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
