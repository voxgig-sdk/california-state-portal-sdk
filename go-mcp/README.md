# california-state-portal-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CaliforniaStatePortal SDK as
two agent tools — `california-state-portal_list` and `california-state-portal_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/california-state-portal-mcp)
make build

# 2. Provide credentials via the environment
export CALIFORNIA_STATE_PORTAL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user california-state-portal \
  -- /absolute/path/to/california-state-portal-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./california-state-portal-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// california-state-portal_list: first page of records
{ "entity": "service" }
{ "entity": "service", "query": { } }

// california-state-portal_load: one record by id
{ "entity": "service", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/california-state-portal-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user california-state-portal \
     -- "$PWD"/dist/*/california-state-portal-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `california-state-portal_list` and `california-state-portal_load` tools now appear
   in new sessions. Ask the agent to *"list service using california-state-portal"*
   and it calls `california-state-portal_list` with `{"entity":"service"}`.
...
```sh
export CALIFORNIA_STATE_PORTAL_APIKEY=sk_live_xxx            # API key
export CALIFORNIA_STATE_PORTAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `california-state-portal_list` tool
...
### Call the `california-state-portal_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `california-state-portal_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `california-state-portal_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CALIFORNIA_STATE_PORTAL_APIKEY` | API key sent with every request. |
| `CALIFORNIA_STATE_PORTAL_BASE` | Optional override of the API base URL. |
...
```sh
./california-state-portal-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"california-state-portal_load","arguments":{"entity":"service","query":{"id":1}}}}'
```
