...
class ServiceDirectTest < Minitest::Test
  def test_direct_list_service
    setup = service_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-service", setup[:live] ? "live" : "unit")
    if _should_skip
...
def service_direct_setup(mockres)
