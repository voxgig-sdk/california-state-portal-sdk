...
class ServiceEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = CaliforniaStatePortalSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    service_ref01_data = nil
    if service_ref01_data_raw.length > 0
...
def service_basic_setup(extra)
  ...
  entid_env_raw = ENV["CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID"]
  ...
  if env["CALIFORNIASTATEPORTAL_TEST_LIVE"] == "TRUE"
    ...
    client = CaliforniaStatePortalSDK.new(Helpers.to_map(merged_opts))
  end
