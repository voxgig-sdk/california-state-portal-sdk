# CaliforniaStatePortal SDK Service entity
...
class ServiceEntity
  ...
  def make
    opts = @_entopts.dup
    ServiceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CaliforniaStatePortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Service, Hash] the current Service data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CaliforniaStatePortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Service fields)
  def match_get
  ...
  #
  # @param reqmatch [ServiceListMatch, Hash, nil] match filter (any subset of
  #   Service fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Service>, Array] the matching Service items; raises CaliforniaStatePortalError on failure
