...
module CaliforniaStatePortalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CaliforniaStatePortalResponse.new({})
    ...
    if fetch_err
      ...
      response = CaliforniaStatePortalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CaliforniaStatePortalResponse.new(fetched)
    else
