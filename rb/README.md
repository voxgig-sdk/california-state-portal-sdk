# CaliforniaStatePortal Ruby SDK



The Ruby SDK for the CaliforniaStatePortal API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Service` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = CaliforniaStatePortalSDK.test(testopts, sdkopts)
```
...
raise a `CaliforniaStatePortalError` (a `StandardError` subclass), so wrap
...
```
rb/
├── CaliforniaStatePortal_sdk.rb       -- Main SDK module
...
The main module (`CaliforniaStatePortal_sdk`) exports the SDK class
