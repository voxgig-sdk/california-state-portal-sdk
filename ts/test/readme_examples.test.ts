...
//     (\`new CaliforniaStatePortalSDK(...)\` / \`CaliforniaStatePortalSDK.test(...)\` become
//     \`CaliforniaStatePortalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CaliforniaStatePortalSDK'
...
const TEST_SEED = {"entity":{"service":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CaliforniaStatePortalSDK(...)\` and any
// \`CaliforniaStatePortalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CaliforniaStatePortalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CaliforniaStatePortalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
