...
describe('ServiceDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CaliforniaStatePortalSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-list-service', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-service', setup.live)) return
