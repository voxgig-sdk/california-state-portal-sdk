...
local ServiceEntity = {}
ServiceEntity.__index = ServiceEntity


function ServiceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ServiceEntity)
  self._name = "service"
  self._client = client
...
function ServiceEntity:get_name()
  return self._name
end


function ServiceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ServiceEntity.new(self._client, opts)
end


function ServiceEntity:data_set(args)
  if args ~= nil then
...
function ServiceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ServiceEntity:match_set(args)
  if args ~= nil then
...
function ServiceEntity:match_get()
...
function ServiceEntity:stream(action, args, callopts)
...
function ServiceEntity:_run_op(ctx, post_done)
  local utility = self._utility
