...
from californiastateportal_types import (
    Service,
    ServiceListMatch,
)
...
class ServiceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ServiceEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Service]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Service().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
