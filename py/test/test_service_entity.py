...
class TestServiceEntity:
    ...
    def test_should_stream(self):
        ...
        base = CaliforniaStatePortalSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID JSON to run live")
        client = setup["client"]
...
def _service_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID"))
    ...
    if env.get("CALIFORNIASTATEPORTAL_TEST_LIVE") == "TRUE":
        ...
        client = CaliforniaStatePortalSDK(helpers.to_map(merged_opts))

    _live = env.get("CALIFORNIASTATEPORTAL_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CALIFORNIASTATEPORTAL_TEST_EXPLAIN") == "TRUE",
        "live": _live,
