...
class TestServiceDirect:
    ...
    def test_should_direct_list_service(self):
        setup = _service_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-service", "live" if setup["live"] else "unit")
        if _skip:
...
def _service_direct_setup(mockres):
    ...
    live = env.get("CALIFORNIASTATEPORTAL_TEST_LIVE") == "TRUE"

    if live:
        merged_opts = {
            "apikey": env.get("CALIFORNIASTATEPORTAL_APIKEY"),
        }
        client = CaliforniaStatePortalSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
