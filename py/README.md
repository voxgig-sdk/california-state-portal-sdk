# CaliforniaStatePortal Python SDK

The Python SDK for the CaliforniaStatePortal API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Service(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.CaliforniaStatePortal(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── californiastateportal_sdk.py         -- Main SDK module
