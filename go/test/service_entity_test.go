...
func TestServiceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func serviceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CALIFORNIASTATEPORTAL_TEST_SERVICE_ENTID")
	...
	if env["CALIFORNIASTATEPORTAL_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewCaliforniaStatePortalSDK(core.ToMapAny(mergedOpts))
	}
