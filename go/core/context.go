...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*CaliforniaStatePortalSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *CaliforniaStatePortalError {
	return NewCaliforniaStatePortalError(code, msg, ctx)
}
