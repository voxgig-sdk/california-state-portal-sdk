# california-state-portal-cli

AQL-driven command-line client **and** interactive REPL for the CaliforniaStatePortal
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/california-state-portal-cli)
make build

# 2. See usage (words, entities, env vars)
./california-state-portal-cli --help
...
./california-state-portal-cli list service

# 5. Override the API base URL for a single call
CALIFORNIA_STATE_PORTAL_BASE=https://api.example.com ./california-state-portal-cli list service

# 6. No arguments -> interactive REPL
./california-state-portal-cli
california-state-portal> list service
california-state-portal> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/california-state-portal-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/california-state-portal-cli list service
   ```
...
Run with no arguments to open a REPL (prompt `california-state-portal>`). Each line is
evaluated as its own AQL expression:

```text
$ ./california-state-portal-cli
california-state-portal> list service
california-state-portal> /help
california-state-portal> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `service`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/california-state-portal-cli`. |
...
(`./california-state-portal-cli <expr>`) and interactively (the REPL), and expressions compose the
