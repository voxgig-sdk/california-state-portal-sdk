// Command CaliforniaStatePortal-cli is a CLI + REPL for the CaliforniaStatePortal SDK,
...
const prompt = "california-state-portal"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "service"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CALIFORNIA_STATE_PORTAL_APIKEY (api key), CALIFORNIA_STATE_PORTAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CALIFORNIA_STATE_PORTAL_APIKEY carries the API key and
	// CALIFORNIA_STATE_PORTAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CALIFORNIA_STATE_PORTAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CALIFORNIA_STATE_PORTAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCaliforniaStatePortalSDK(opts)
